 ...
 * @return {*}
 */
export const genResponseData = <T>(
  success: boolean,
  msg?: string,
  data?: T
) => {
 ...
 * @return {*}
 */
export const genMd5EncryptedPassword = (
  username: string,
  password: string,
  salt: string
) => {
...
export const getTIDByReportData = (data: string) => {
  ...
  const TIDStart =
    8 + MidCommandLength + TIDLengthCommandLength + EPCLengthStart;
  ...
  const TIDLength =
    parseInt(
      '0x' +
        str.substring(
          EPCLengthStart + EPCLength,
          EPCLengthStart + EPCLength + TIDLengthCommandLength
        ),
      16
    ) * 2;
...
export const convertDecimalToBinary = (number: number) => {
  ...
  const MAX_LENGTH = 8;
  const binaryString =
    new Array(MAX_LENGTH - binaryLength).fill('0').join('') + binary;
