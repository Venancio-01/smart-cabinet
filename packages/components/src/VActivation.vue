<script lang="ts" setup>
import { onMounted, ref } from 'vue'
import { useVModel } from '@vueuse/core'
import VIcon from './VIcon.vue'

interface Props {
  registrationCode: string
  activationCode: string
}

const props = withDefaults(defineProps<Props>(), {
  registrationCode: '',
  activationCode: '',
})
const emit = defineEmits(['update:activationCode'])

const activationCodeValue = useVModel(props, 'activationCode', emit)
const inputEl = ref<null | HTMLInputElement>(null)

const labelWord = '请输入激活码'
const labelWordArr = labelWord.split('')

function handleFocus() {
  inputEl.value?.focus()
}

onMounted(() => {
  handleFocus()
})
</script>

<template>
  <div class="flex flex-col items-center">
    <div class="flex justify-center mt-[120px]">
      <VIcon icon="activate" class="text-[200px] text-light" />
    </div>

    <div class="w-[500px] text-lg text-light mt-[20px]">
      <span>注册码：</span>
      <span class="select-text">{{ registrationCode }}</span>
    </div>

    <div class="flex-center-center">
      <div m="y-40px" w="500px" flex="~" justify-center relative class="form-control">
        <input ref="inputEl" v-model="activationCodeValue" type="password" required>
        <label>
          <span v-for="(item, index) in labelWordArr" :key="index" :style="{ transitionDelay: `${index * 20}ms` }">{{ item }}</span>
        </label>
      </div>
    </div>
  </div>
</template>

<style scoped>
.form-control input {
  background-color: transparent;
  border: 0;
  border-bottom-width: 2px;
  border-style: solid;
  display: block;
  width: 100%;
  padding: 15px 0;
  font-size: 18px;
  @apply w-full border-light text-light;
}

.form-control input:focus,
.form-control input:valid {
  outline: 0;

  @apply border-b-light;
}

.form-control label {
  position: absolute;

  top: 15px;

  left: 0;

  pointer-events: none;
}

.form-control label span {
  @apply text-light;

  display: inline-block;

  font-size: 18px;

  min-width: 5px;

  transition: 0.3s cubic-bezier(0.68, -0.55, 0.265, 1.55);
}

.form-control input:focus + label span,
.form-control input:valid + label span {
  @apply text-light;

  transform: translateY(-30px);
}
</style>
