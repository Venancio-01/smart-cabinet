...
import {
  addFinger,
  queryFingerByUserIdAndOrder,
  updateFingerByUserIdAndOrder,
} from "./database";
import {
  MAX_DEVICE_NUM,
...
export function openDevice() {
  ...
  const success =
    deviceHandle.deref() !== null && algorithmHandler.deref() !== null;
  isOpen = success;
...
export function startFingerCapture() {
  ...
  // 获取指纹仪捕获到的图像
  const result = captureFingerImage(
    deviceHandle,
    imageBuffer,
  ...
  // 提取图像
  const templateDataLen = extractTemplate(
    algorithmHandler,
    imageBuffer,
    deviceWidth,
    deviceHeight,
...
export async function onRegister(templateData, userId, order) {
  ...
  if (registerCurrentIndex > 0) {
    // 对比前后两次采集的指纹
    const success = verifyTemplate(
      algorithmHandler,
      registerTemplates[registerCurrentIndex - 1],
      templateData
    );
    if (!success) {
      resetRegisterData();
  ...
  if (registerCurrentIndex !== MAX_REGISTRATION_COUNT)
    return genResponseData(
      true,
      `您还需要按压${MAX_REGISTRATION_COUNT - registerCurrentIndex}次手指`
    );
  ...
  if (!genTempSuccess) {
    ...
    return genResponseData(
      false,
      `生成登记模板失败，错误代码 = ${genTempResult}`,
      { alert: true }
    );
  }
  const { success: addDbSuccess, result } = addTemplateToDb(
    algorithmHandler,
    9999,
    genTempResult,
    registerTemplateData
