...
const misPlaceCarriers = computed(() => {
  return misPlaceCarrierData.value.filter(
    (item) => Number(item.cabinetDoorId) === cabinetDoor.value.id
  );
});
...
<template>
  <div
    class="rounded-2xl bg-white bg-opacity-20 text-light flex h-full items-center"
    @click="() => openCabinetDoor(cabinetDoor)"
  >
    <BaseIcon
      icon="door"
      class="text-[240px] flex-1"
