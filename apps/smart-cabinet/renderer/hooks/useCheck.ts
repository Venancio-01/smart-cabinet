...
export default function () {
  ...
  const { updateCarrier, recordDataWhenCheckEnd, recordDataWhenCheckStart } =
    useCarrier();
  const {
    resetCountdowns,
  ...
  const generateCheckResult = () => {
    ...
    const result: CheckResultType[] = cabinetDoorList.value.map((door) => {
      const currentDoorBorrowCarriers = borrowCarriers.filter(
        (item) => item.cabinetDoorId === door.id
      );
      const currentDoorReturnCarriers = returnCarriers.filter(
        (item) => item.cabinetDoorId === door.id
      );

      // 如果是本次操作的柜门，则显示错放文件数据
      const isOperationCabinetDoor = lastOperationCabinetDoorList.value.find(
        (item) => item.id === door.id
      );
      const currentDoorMisPlaceCarrierRecords = isOperationCabinetDoor
        ? misPlaceCarrierRecords.filter(
  ...
  const handleCheck = async (doorId: number) => {
    ...
    const currentDoor = cabinetDoorList.value.find(
      (item) => item.id === doorId
    );
    ...
    const stopCheckCountdown = await startCheckCountdown(
      currentDoor.id,
      async () => {
        stopCheckCountdown();
  ...
  const handleManualCheck = () => {
    ...
    const noAvailableRfid = cabinetDoorList.value.every(
      (item) => !item.rfidIsConnected
    );
    if (noAvailableRfid) {
    ...
    const availableCabinetDoors = cabinetDoorList.value.filter(
      (item) => item.rfidIsConnected
    );
