...
export async function handleConnect() {
  ...
  if (address === null || port === null || txid === null)
    return

  const messageQueue = new MessageQueue()
  const socket = new Socket({ address, port: Number(port), message: messageQueue })
...
export async function registerMessageListerner(equipment: DoorEquipment, message: MessageQueue) {
  message.on<[Message]>('push', async (msg) => {
    ...
    // 读到 EPC 标签
      ...
      if (carriers.length === 0 || registrationRecords.length === 0)
        return
