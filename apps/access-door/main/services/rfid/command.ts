 ...
 * @return {*}
 */
export function generteSetGPITriggerCommand(
  GPIIndex: GPIIndexType,
  ...
  // 触发命令的长度
  const triggerCommandLength = (triggerCommand.length / 2)
    .toString(16)
    .padStart(4, "0");
  ...
  // 停止延时时间， mid = 01
  const duration = `01${(DETECTION_DURATION / 10)
    .toString(16)
    .padStart(4, "0")}`;
