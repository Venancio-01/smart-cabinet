...
async function loadAccessRecords() {
  const { data: _data, total: _total } = await fetchAccessRecords(
    toRaw(condition)
  );
  data.value = _data;
  total.value = _total;
    ...
    <div class="flex" m="t-8" p="b-4">
      ...
      >
        <a-form-item label="出入方向" name="title">
          <a-select
            v-model:value="condition.accessDirection"
            allow-clear
            placeholder="请选择出入方向"
            @change="handleQuery"
          >
        ...
        <a-form-item label="出入时间" name="title">
          <a-select
            v-model:value="condition.timeRange"
            allow-clear
            placeholder="请选择出入时间"
            @change="handleQuery"
          >
            <a-select-option
              v-for="item in AccessTimeRangeMap"
              :key="item.value"
              :value="item.value"
            >
              {{ item.label }}
        ...
        <a-form-item label="是否告警" name="title">
          <a-select
            v-model:value="condition.hasAlarm"
            allow-clear
            placeholder="请选择是否告警"
            @change="handleQuery"
          >
        ...
        <a-form-item label="携带载体" name="title">
          <a-select
            v-model:value="condition.withCarrier"
            allow-clear
            placeholder="请选择是否携带载体"
            @change="handleQuery"
          >
            <a-select-option :value="AccessWithCarrier.ALL">
              全部
            </a-select-option>
            <a-select-option :value="AccessWithCarrier.WITH_CARRIER">
              是
            </a-select-option>
            <a-select-option :value="AccessWithCarrier.WITHOUT_CARRIER">
              否
            </a-select-option>
          </a-select>
        </a-form-item>
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.dataIndex === 'accessDirection'">
            <span>{{
              record.accessDirection === 1
                ? "进入"
                : record.accessDirection === 2
                ? "外出"
                : ""
            }}</span>
          </template>
          ...
          <template v-if="column.dataIndex === 'has_alarm'">
            <span>{{
              record.has_alarm === 0 ? "否" : record.has_alarm === 1 ? "是" : ""
            }}</span>
          </template>

          <template v-if="column.dataIndex === 'action'">
            <a-button
              v-if="record.carrier_count > 0"
              type="link"
              size="small"
