...
<template>
  <div class="col-span-2 grid grid-flow-col grid-rows-4 grid-cols-4 gap-4">
    <CabinetDoor
      v-for="(item, index) in doorList" :key="index" :index="index" :name="item.viewName || ''"
      :in-place-number="item.inPlaceCarrierCount" :mis-place-number="item.misPlaceCarrierCount"
      :total-number="item.totalCarrierCount" department-name="" @click="openCabinetDoor(item)"
    />
  </div>
</template>
