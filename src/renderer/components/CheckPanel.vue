...
const checkCountdown = computed(() => {
  return cabinetDoorList.value.find(
    (item) => item.id === currentCheckCabinetDoorId.value
  )?.checkCountdown;
});
  ...
  <transition mode="out-in">
    ...
    >
      ...
      >
        ...
        <span
          class="absolute top-1/2 left-1/2 -translate-x-1/2 font-['Barlow'] text-[90px] mr-4"
          >{{ checkCountdown || 10 }}</span
        >
        <span
          class="absolute top-1/2 left-1/2 -translate-x-1/2 ml-[60px] mt-[30px]"
          >秒</span
        >
      </div>
    </div>
